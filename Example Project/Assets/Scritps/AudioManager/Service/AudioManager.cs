...
namespace AudioManager.Service {
    public class AudioManager : IAudioManager {
        ...
        public AudioError PlayAtTimeStamp(string name, float startTime) {
            ...
            m_parentBehaviour.StartCoroutine(DetectCurrentProgressCoroutine(name, Constants.MAX_PROGRESS, m_resetStartTimeCallback));
            source.Play();
            return error;
        }
        ...
        private IEnumerator DetectCurrentProgressCoroutine(string name, float progress, AudioFinishedCallback callback) {
            ...
            yield return new WaitUntil(() => source.SoundFinished());
            float remainingTime = source.ConvertProgressIntoTimeStamp(progress);
        ...
        private AudioError CreateNewChildren(AudioSource parentSource, Vector3 position, string keyName, out AudioSource newChildSource) {
            AudioError error = parentSource.CreateEmptyGameObject(keyName, position, m_parentTransform, out newChildSource);
